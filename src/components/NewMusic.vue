...
<div>
    <ul class="newMusiclist">
        <li v-for="v in newMusicList" :key="v.id" tag="li"  @click="changesongs(v.id)">
            <div class="left">
                <div>{{ v.name }}</div>
                <p :class="v.song.privilege.playMaxbr > 320000?'sq':''">{{ v.song.artists[0].name }}-{{ v.name }}</p>
            </div>
            <div class="right">
                <span></span>
            </div>
...
export default {
    ...
    methods:{
        changesongs(ID){
            this.$store.commit('changePlayList' , this.newMusicList)
